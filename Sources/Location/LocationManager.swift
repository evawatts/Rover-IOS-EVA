...
class LocationManager: NSObject {
    ...
    init(
        ...
        if maxGeofenceRegionsToMonitor > theoreticalMaximumGeofences {
            fatalError(
                "You may only specify that Rover can monitor up to \(theoreticalMaximumGeofences) geofences at a time.")
        }
        if maxBeaconRegionsToMonitor >= maxGeofenceRegionsToMonitor {
            fatalError(
                "Rover uses the same region slots for monitoring beacons as it does for monitoring geofences, so therefore you may only specify that Rover can monitor up to the same max number of beacons as geofence regions, currently \(maxGeofenceRegionsToMonitor)."
            )
        }
...
extension LocationManager: RegionManager {
    ...
    func updateMonitoredRegions(manager: CLLocationManager) {
        let beaconRegions = Set<CLRegion>(
            Beacon.wildcardRegions(
                in: self.context,
                maxLength: self.maxBeaconRegionsToMonitor
            ).map { $0 as CLRegion }
        )
        os_log(
            "Monitoring for %d wildcard (UUID-only) beacon regions", log: .location, type: .debug, beaconRegions.count)

        let circularRegions = Set<CLRegion>(
            Geofence.regions(
                in: self.context,
                closestTo: manager.location?.coordinate,
                maxLength: self.maxGeofenceRegionsToMonitor - beaconRegions.count
            ).map { $0 as CLRegion }
        )
    ...
    func enterGeofence(region: CLCircularRegion) {
        ...
        let regionIdentifier = region.untaggedIdentifier(tag: "Rover")
        var enterEvent: EventInfo?
        self.context.performAndWait {
            guard
                let snapshot = Geofence.fetchSnapshotOnContextQueue(
            ...
            guard let geofence = Geofence.resolveOnContextQueue(snapshot, in: self.context) else {
                return
            }
            ...
            enterEvent = geofence.enterEvent
        }

        if let enterEvent {
            eventQueue.addEvent(enterEvent)
        }
    }
    ...
    func updateNearbyBeacons(_ beacons: [CLBeacon], in region: CLBeaconRegion, manager: CLLocationManager) {
        ...
        let nextSnapshots = Beacon.fetchSnapshots(
            matchingRegionIdentifiers: identifiers,
            in: self.context
        )

        var enteredEvents = [EventInfo]()
        var exitedEvents = [EventInfo]()
        var hasUpdates = false

        self.context.performAndWait {
            let previousBeacons = self.beaconMap[region] ?? Set<Beacon>()
            let previousObjectIDs = Set(previousBeacons.map(\.objectID))
            let nextObjectIDs = Set(nextSnapshots.map(\.objectID))

            guard previousObjectIDs != nextObjectIDs else {
                os_log("Nearby beacons already up to date", log: .location, type: .debug)
                return
            }

            let nextBeacons = Beacon.resolveOnContextQueue(nextSnapshots, in: self.context)
            self.beaconMap[region] = nextBeacons

            let previousByObjectID = Dictionary(uniqueKeysWithValues: previousBeacons.map { ($0.objectID, $0) })
            let nextByObjectID = Dictionary(uniqueKeysWithValues: nextBeacons.map { ($0.objectID, $0) })

            enteredEvents =
                nextObjectIDs
                .subtracting(previousObjectIDs)
                .compactMap { nextByObjectID[$0] }
                .map {
            ...
            exitedEvents =
                previousObjectIDs
                .subtracting(nextObjectIDs)
                .compactMap { previousByObjectID[$0] }
                .map {
                    os_log("Exited beacon: %@", log: .location, type: .debug, $0)
                    return $0.exitEvent
                }
        ...
        var beaconsForObservers = Set<Beacon>()
        self.context.performAndWait {
            beaconsForObservers = self.currentBeacons
        }
        beaconObservers.notify(parameters: beaconsForObservers)
    }
}
